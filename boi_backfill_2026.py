...
def main():
    ...
    # ak-5oi flags fix: --start / --end are shorter aliases for
    # --date-from / --date-to so infra can re-fire a scoped window
    # without hand-editing (per Lead hq-wisp-u2pnf0). Both forms
    # write to the same `args.date_from` / `args.date_to` via dest=;
    # argparse resolves the last-supplied wins.
    # Accepts either YYYY/MM/DD (Gmail native) or YYYY-MM-DD; we
    # normalize dashes to slashes at the bottom of main() before
    # threading into route_download_process.
    parser.add_argument(
        "--start", "--date-from", dest="date_from",
        default=DEFAULT_DATE_FROM,
        help=(
            f"Window start date (default: {DEFAULT_DATE_FROM}). "
            f"Accepts YYYY/MM/DD or YYYY-MM-DD."
        ),
    )
    parser.add_argument(
        "--end", "--date-to", dest="date_to",
        default=DEFAULT_DATE_TO,
        help=(
            f"Window end date, exclusive (default: {DEFAULT_DATE_TO}). "
            f"Accepts YYYY/MM/DD or YYYY-MM-DD."
        ),
    )
    parser.add_argument(
    ...
    # ak-5oi flags fix: normalize YYYY-MM-DD → YYYY/MM/DD for Gmail's
    # after:/before: operators (StatementDownloadService._fetch_emails
    # uses these raw). Callers may pass either format; Gmail insists
    # on slashes downstream.
    args.date_from = args.date_from.replace("-", "/")
    args.date_to = args.date_to.replace("-", "/")

    print("ak-5oi BOI backfill")
