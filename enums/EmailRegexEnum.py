...
class EmailRegexEnum(Enum):
    ...
    YES_BANK = r"INR (?P<amount_spent>[\d,]+(?:\.\d+)?) has been spent on your YES BANK Credit Card ending with (" \
               r"?P<card_number>\d{4}) at (?P<merchant>.+?) on (?P<transaction_date>[\d-]+) at (?P<transaction_time>[" \
